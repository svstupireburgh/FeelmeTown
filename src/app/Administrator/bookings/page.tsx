...
export default function BookingsPage() {
  ...
  const fetchBookings = async (silent = false) => {
    try {
      ...
      const [confirmedBookingsResponse, completedBookingsResponse, manualBookingsResponse, cancelledBookingsResponse, incompleteBookingsResponse] = await Promise.all([
        ...
        fetch('/api/admin/manual-bookings'), // Manual bookings from JSON file
      ...
      const completedBookings = completedArray
        ...
        return {
          ...
          venuePaymentMethod: booking.venuePaymentMethod || booking.paymentMethod || booking.finalPaymentMethod || null,
          bookingId: booking.bookingId || booking.booking_id || booking.bookingID,
          id: booking.bookingId || booking.booking_id || booking.bookingID,
  ...
  const sortedBookings = filteredBookings.sort((a, b) => {
    ...
    // Confirmed bookings always on top (must run BEFORE date sorting)
    ...
    // Confirmed: today first, then future dates ascending (time ascending within same day)
  ...
  const handleTogglePaymentStatus = (booking: any) => {
    const bookingId = String(booking.originalBookingId || booking.bookingId || booking.id).replace(/^#/, "");
    if (!bookingId) {
      showError('Booking ID missing');
      return;
    }
                  ...
                  <tr key={`${booking.id}-${index}`}>
                    ...
                    <td>
                      ...
                      >
                        {paymentBadge.animate ? (
                          ...
                          <span className="badge-line">{paymentBadge.secondaryText || paymentBadge.primaryText}</span>
                        )}
                      </span>
                    </td>
                    ...
                    <td>
                      <div className="action-buttons">
                        {String(booking.status || '').toLowerCase() !== 'completed' && canTogglePayment && (
                          isPaid ? (
                            <button
                            ...
                            <>
                              <button
                                className={`action-btn pay-btn ${sendingInvoiceId === (booking.originalBookingId || booking.bookingId || booking.id) ? 'loading' : ''}`}
                                title="Send Invoice"
                                onClick={() => handleSendInvoiceOnly(booking)}
                                disabled={sendingInvoiceId === (booking.originalBookingId || booking.bookingId || booking.id)}
                              >
                                {sendingInvoiceId === (booking.originalBookingId || booking.bookingId || booking.id) ? 'Sending…' : 'Invoice'}
                              </button>
                              <button
